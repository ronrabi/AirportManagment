...
int saveFlightToBinaryFileCompressed(Flight* pFlight, FILE* fp)
{
	BYTE data[6] = { 0 };
	...
	data[0] = (sourceLen << 3) | (destLen >> 2);
	data[1] = (destLen << 6) | (pFlight->thePlane.type << 4) | pFlight->date.month;
...
int loadFlightFromFileCompress(Flight* pFlight, FILE* fp)
{
	...
	int sourceLen = (data[0] >> 3) & 0x1F;
	int destLen = ((data[0] << 2) & 0x1F) | ((data[1] >> 6) & 0x3);
	...
	if (fread(pFlight->nameSource, sizeof(char), sourceLen, fp) != sourceLen)
	{
	...
	if (fread(pFlight->nameDest, sizeof(char), destLen, fp) != destLen)
	{
		free(pFlight->nameSource);
