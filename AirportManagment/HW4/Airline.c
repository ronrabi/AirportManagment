...
int		saveAirlineToBinaryFileCompressed(const Airline* pComp, const char* fileName)
{
	FILE* fp;
	...
	{
		if (!saveFlightToBinaryFileCompressed(pComp->flightArr[i], fp))
		{
			fclose(fp);
			return 0;
		}
...
int		initAirlineFromFile(Airline* pComp, AirportManager* pManaer, const char* fileName)
{
	...
	if (fileName == AIRLINE_CONPRESS_FILE_NAME)
	{
		if (!loadAirlineFromBinaryFileCompressed(pComp, fileName))
			return 0;
	}
	else
...
int		loadAirlineFromBinaryFileCompressed(Airline* pComp, const char* fileName)
{
	FILE* fp;
	...
	int nameLen = data[1] & 0xF;
	pComp->flightCount = (data[0] << 1) | ((data[1] >> 7) & 0x1);
	pComp->sortOpt = (data[1] >> 4) & 0x7;

	pComp-> name = (char*)calloc(nameLen + 1, sizeof(char));
	if (!pComp->name)
	{
		fclose(fp);
		return 0;
	...
	for (int i = 0; i < pComp->flightCount; i++)
	{
		...
		if (!loadFlightFromFileCompress(pComp->flightArr[i],fp))
		{
			free(pComp->flightArr);
			fclose(fp);
			return 0;
